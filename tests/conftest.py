...
def tmp_conn(tmp_path):
    ...
    in isolation, without touching the real db/portfolio.db or needing real Excel files."""
    from etl import db
...
def real_conn():
    ...
    if not os.path.exists(REAL_DB_PATH):
        pytest.skip(f"db/portfolio.db not found at {REAL_DB_PATH} -- run scripts/load_data.py first")
